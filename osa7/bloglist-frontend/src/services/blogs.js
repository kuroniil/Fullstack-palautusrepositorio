...
const newBlog = async (title, author, url, token) => {
  ...
  const response = await axios.post(baseUrl, blogData, { headers: { 'Authorization': `Bearer ${token}` } })
...
const deleteBlog = async (blogId, token) => {
  const response = await axios.delete(`/api/blogs/${blogId}`, { headers: { 'Authorization': `Bearer ${token}` } })
