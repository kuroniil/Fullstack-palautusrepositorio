  ...
  if (showBlogInfo !== true) {
    return (
      <div style={BlogStyle} className="togglableContent">
        ...
        <button onClick={() => {handleBlogInfoClick(blog.id)}}>view</button>
      </div>
    )
    ...
    return (
      <div style={BlogStyle}>
        <p>{blog.title}</p>
        <p>{blog.url}</p>
        <p>likes {blog.likes} <button onClick={() => {handleLikeClick(blog)}}>like</button></p>
        <p>{blog.author}</p>
        <button onClick={() => {handleRemoveClick(blog.id)}} style={ButtonStyle}>remove</button>
        <button onClick={() => {handleBlogInfoClick(blog.id)}}>hide</button>
      </div>
    )
  } else {
    return (
